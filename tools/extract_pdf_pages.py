...
def render_pdf_pages(
    ...
    for index in range(doc.page_count):
        ...
        if force or not out_path.exists():
            ...
            # Avoid importing image libraries just to read dimensions on reruns.
            page = doc.load_page(index)
        ...
        pages.append(
            {
                "page": page_number,
                "file": f"pages/png/page{page_number:03d}.png",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract the Warsh muthamma PDF pages to app-compatible PNG files."
    )
    parser.add_argument(
        "--pdf",
        default="source/warsh-muthamma-source.pdf",
        help="PDF path relative to the repository root.",
    )
    parser.add_argument(
        "--out",
        default="pages/png",
    ...
    index_path = repo_root / "pages" / "pages_png_index.json"
